...
import { Eye, EyeOff, Loader2, ArrowLeft, Lock, Mail, User, Phone, Check } from "lucide-react";
...
export default function RegisterPage() {
  ...
  const passwordChecks = [
    { label: locale === "ar" ? "8 أحرف على الأقل" : "Min 8 characters", met: formData.password.length >= 8 },
    { label: locale === "ar" ? "يحتوي على رقم" : "Contains a number", met: /\d/.test(formData.password) },
    { label: locale === "ar" ? "يحتوي على حرف كبير" : "Contains uppercase", met: /[A-Z]/.test(formData.password) },
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) { setError(data.error || (locale === "ar" ? "حدث خطأ أثناء إنشاء الحساب" : "Registration error")); return; }
  ...
  const fields = [
    { key: "name", label: t("name"), type: "text", placeholder: locale === "ar" ? "الاسم الكامل" : "Full name", icon: User, dir: "rtl" as const },
    { key: "email", label: t("email"), type: "email", placeholder: "email@example.com", icon: Mail, dir: "ltr" as const },
    { key: "phone", label: `${t("phone")} (${locale === "ar" ? "اختياري" : "optional"})`, type: "tel", placeholder: "+212 6XX-XXXXXX", icon: Phone, dir: "ltr" as const, optional: true },
  ];
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-secondary relative overflow-hidden items-center justify-center">
        ...
        >
          ...
          <p className="text-gray-400 text-lg mb-8">
            {locale === "ar" ? "أنشئ حسابك واستمتع بتجربة تسوق فريدة" : "Create your account and enjoy a unique shopping experience"}
          </p>
          <div className="space-y-4 text-left max-w-xs mx-auto">
            {[
              locale === "ar" ? "عروض حصرية للأعضاء" : "Exclusive member offers",
              locale === "ar" ? "تتبع طلباتك بسهولة" : "Easy order tracking",
              locale === "ar" ? "سرعة في الخدمة" : "Fast service",
            ].map((benefit, i) => (
              <motion.div
                key={i}
        ...
        >
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
            ...
            <p className="text-gray-500 mb-8">
              {locale === "ar" ? "املأ البيانات لإنشاء حسابك" : "Fill in your details to create your account"}
            </p>
          </motion.div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {fields.map((field, i) => (
              <motion.div key={field.key} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + i * 0.05 }}>
                <label className="block text-sm font-medium text-gray-700 mb-2">{field.label}</label>
                <div className="relative">
                  <field.icon size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
                  <input
                    type={field.type} required={!field.optional}
            ...
            >
              {loading ? <><Loader2 size={18} className="animate-spin" /> {locale === "ar" ? "جاري الإنشاء..." : "Creating..."}</> : t("submit")}
            </motion.button>
          </form>
          ...
          <p className="text-center mt-8 text-sm text-gray-500">
            {t("hasAccount")}{" "}
            <Link href={L("/login")} className="text-gold hover:text-gold-dark font-semibold transition-colors">{t("login")}</Link>
          </p>
